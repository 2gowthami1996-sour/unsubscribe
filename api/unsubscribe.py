...
def handler(request):
    ...
    try:
        ...
        return {"statusCode": 500, "body": f"❌ DB connection failed: {e}"}

    # Mark unsubscribed
    result = collection.update_one({"work_emails": {"$regex": email}}, {"$set": {"unsubscribed": True}})

    if result.matched_count > 0:
        return {"statusCode": 200, "body": f"{email} has been unsubscribed successfully!"}
    else:
        return {"statusCode": 404, "body": f"No matching email found for {email}."}
